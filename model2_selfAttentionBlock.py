...
# PROBLEM:  This implementation is quite a deep neural network for our task.
#           Therefore, it runs into optimization issues. This document is just to
#           highlight the process--avoid running this network.
#           The next model implements RESIDUAL BLOCKS which is what we want.
...
class FeedForward(nn.Module):
    def __init__(self, n_embd):
        ...
        self.net = nn.Sequential(
            nn.Linear(n_embd, n_embd),      # Linear (n_embd x n_embd) layer
...
class Block(nn.Module):
    ...
    def forward(self, x):
        x = self.sa(x)          # Passing through self-attention (multi-head)
        x = self.fforward(x)    # Passing through feed-forward layer
        return x
